...
Returns: a tuple of (predictions, image_paths):
    ...
    image_paths: a list of image paths corresponding to each prediction in predictions """
        predictions = []
        image_paths = []
        imges, image_paths = self.load_images(folder_path)
        for image, path in zip(imges, image_paths):
            pimages, image_shape = self.preprocess_images([image])
            outputs = self.model.predict(pimages)
            boxes, box_confidences, box_class_probs = self.process_outputs(outputs, image_shape[0])
            ...
            self.show_boxes(image, final_boxes, final_classes, final_scores, file_name)
