...
def bag_of_words(sentences, vocab=None):
    ...
    def normalize_text(text):
        ...
        # Only remove 's' if word ends in 's' and is longer than 3 chars
        # BUT not if it's an irregular plural or word that naturally ends in 's'
    ...
    if vocab is None:
        ...
        for sentence in sentences:
            # Remove punctuation (including possessive 's)
            sentence = re.sub(r"[^a-zA-Z\s]", "", sentence)
            # lower case and split
            words = sentence.lower().split()
            # normalize - remove trailing 's'
